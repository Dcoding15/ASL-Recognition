...
from models import TCN, FrameFeatureEncoder

MAX_FRAMES = 10  # Fixed sequence length
IMAGE_SIZE = 32  # Resize images to 32x32 for memory efficiency
FEATURE_DIM = 512  # Reduced feature dimension per frame
...
class SequenceImageDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        frames = frames.view(MAX_FRAMES, -1)      # Flatten each frame -> (T, C*H*W)
...
input_dim = 3 * IMAGE_SIZE * IMAGE_SIZE  # flattened frame size
...
# --------------------------
# Model: Frame encoder + TCN
# --------------------------
frame_encoder = FrameFeatureEncoder(input_dim=input_dim, hidden_dim=FEATURE_DIM)
model = nn.Sequential(
    frame_encoder,
    TCN(num_inputs=FEATURE_DIM, num_channels=[256, 256, 256], num_classes=num_classes)
)
...
#torch.save(model.state_dict(), 'pre trained weigths/tcn_weights.pth')
